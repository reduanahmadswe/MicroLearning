...
export default function EditLessonPage() {
  ...
  const fetchLesson = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`https://microlearning-backend-reduan.onrender.com/api/v1/lessons/${lessonId}`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(
        `https://microlearning-backend-reduan.onrender.com/api/v1/lessons/${lessonId}`,
        lessonData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
